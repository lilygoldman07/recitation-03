...
For a constant fraction of these levels, the work is still $\Theta(n \log n)$, so the total work is $\Theta(n \log^2 n)$.



c) $W(n) = 2W(n/2)+ n/ \log n$
**Answer: $W(n) \in \Theta(n \log \log n)$**
...
**Answer: root-dominated, so $W(n) \in \Theta(n)$**
...
**Answer: leaf-dominated, so $W(n) \in \Theta(n)$**
